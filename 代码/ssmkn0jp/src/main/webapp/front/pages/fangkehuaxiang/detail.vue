<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"140rpx","fontSize":"28rpx","color":"#333","textAlign":"right"}'>年龄：</view>
					<view class="text" :style='{"color":"#333","textAlign":"right","fontSize":"28rpx"}'>{{detail.nianling}}</view>
				</view>
                				                				<view class="detail-list-item" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"#fff","backgroundColor":"#fff","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"68rpx"}'>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','fangkehuaxiang');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','fangkehuaxiang');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('fangkehuaxiang', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussfangkehuaxiang/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('fangkehuaxiang', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('fangkehuaxiang', this.detail);
				this.$utils.msg('审核成功');
