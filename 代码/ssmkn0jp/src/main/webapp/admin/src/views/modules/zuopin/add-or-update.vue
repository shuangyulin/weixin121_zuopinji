<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="作品名称" prop="zuopinmingcheng">
          <el-input v-model="ruleForm.zuopinmingcheng" 
              placeholder="作品名称" clearable  :readonly="ro.zuopinmingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="作品名称" prop="zuopinmingcheng">
              <el-input v-model="ruleForm.zuopinmingcheng" 
                placeholder="作品名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="作品分类" prop="zuopinfenlei">
        ...
        <div v-else>
          <el-form-item class="input" label="作品分类" prop="zuopinfenlei">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.zuopinfengmian" label="作品封面" prop="zuopinfengmian">
          <file-upload
          tip="点击上传作品封面"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.zuopinfengmian" label="作品封面" prop="zuopinfengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zuopinfengmian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="作品内容" prop="zuopinneirong">
          <file-upload
          tip="点击上传作品内容"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.zuopinneirong?ruleForm.zuopinneirong:''"
          @change="zuopinneirongUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.zuopinneirong" label="作品内容" prop="zuopinneirong">
            <el-button type="text" size="small" @click="download(ruleForm.zuopinneirong)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="作品简介" prop="zuopinjianjie">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="作品简介"
                  v-model="ruleForm.zuopinjianjie" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zuopinjianjie" label="作品简介" prop="zuopinjianjie">
                    <span>{{ruleForm.zuopinjianjie}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zuopin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zuopinCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    zuopinneirongUploadChange(fileUrls) {
	this.ruleForm.zuopinneirong = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
